...
function getFrequency(note: string, octave = 4): number {
  const fullNote = `${note}${octave}`;
  return NOTE_FREQUENCIES[fullNote] || NOTE_FREQUENCIES[`${note}4`] || 440;
}
...
export function usePlayNote() {
  ...
  const playNote = useCallback((note: string, octave = 4, duration = 0.5) => {
